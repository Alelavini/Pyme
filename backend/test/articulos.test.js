...
let token;
beforeAll(async () => {
  const res = await request(app).post("/api/login").send({ usuario: "admin", clave: "123" });
...
describe("POST /api/articulos", () => {
  it("Deberia devolver el articulo que acabo de crear", async () => {
    const res = await request(app).post("/api/articulos").set("Authorization", `Bearer ${token}`).send(articuloAlta);
...
describe("DELETE /api/articulos/:id", () => {
  it("Debería devolver el artículo con el id 1 borrado", async () => {
    const res = await request(app).delete("/api/articulos/1").set("Authorization", `Bearer ${token}`);
    expect(res.statusCode).toEqual(200);
...
describe("Escrituras sin token", () => {
  it("Debería rechazar el alta de un artículo sin autenticación", async () => {
