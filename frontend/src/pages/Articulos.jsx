...
export default function Articulos() {
  ...
  const [categorias, setCategorias] = useState({}) // IdCategoria -> Nombre
  ...
  useEffect(() => {
    apiFetch('/api/categorias')
      .then((data) => setCategorias(Object.fromEntries(data.map((c) => [c.IdCategoria, c.Nombre]))))
      .catch(() => {})
  }, [])
            ...
            <tbody>
              {articulos.length === 0 ? (
                ...
                <tr key={a.IdArticulo}>
                  ...
                  <td>{categorias[a.IdCategoria] ?? a.IdCategoria}</td>
                  <td>{a.Stock}</td>
                  <td>
